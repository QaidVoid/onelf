...
pub fn pack(opts: &PackOptions, runtime_binary: &[u8]) -> io::Result<()> {
    ...
    let dict = if opts.use_dict && files.len() > 1 && total_content_size > 4096 {
        ...
        let dict_size = 112_640.min(total_content_size / 2);
    ...
    let compressed_files: Vec<CompressedFile> = files
        ...
        .map(|f| {
            ...
            let total_original_size = f.content.len() as u64;
    ...
    let lib_dir_offsets: Vec<u32> = opts.lib_dirs.iter().map(|d| strings.add(d)).collect();
